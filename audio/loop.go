...
func NewInfiniteLoop(src ReadSeekCloser, length int64) *InfiniteLoop {
	return NewInfiniteLoopWithIntro(src, 0, length/bytesPerSample)
}

func NewInfiniteLoopWithIntro(src ReadSeekCloser, introLengthInSamples int64, loopLengthInSamples int64) *InfiniteLoop {
	return &InfiniteLoop{
		src:     src,
		lstart:  introLengthInSamples * bytesPerSample,
		llength: loopLengthInSamples * bytesPerSample,
		pos:     -1,
	}
}
